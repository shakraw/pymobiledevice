    ...
    def handle(self):
        while True:
            ...
            if self.b in rlo:
                ...
                self.btoa += s
                # logging.debug(
                #   "Relay iter: %8d atob, %8d btoa, lists: %r %r %r"%(len(self.atob), len(self.btoa), rlo, wlo, xlo)
                # )


class TCPRelay(socketserver.BaseRequestHandler):
    def handle(self):
        logging.debug("Incoming connection to %d" % self.server.server_address[1])
...
for rport, lport in ports:
    logging.debug("Forwarding local port %d to remote port %d" % (lport, rport))
